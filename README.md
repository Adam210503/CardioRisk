...
├── main.py                           # FastAPI REST scoring API
├── train_model.py                    # Full training pipeline
├── optimized_heart_disease_model.pkl # Serialised model (generated by train_model.py)
├── requirements.txt
└── README.md
```
...
- The Cleveland dataset contains 303 samples. This is small by modern ML standards; the model's generalisation to broader patient populations is unverified.
- Class 4 (critical disease) has only 13 real samples. SMOTE generates synthetic examples to compensate, but predictions at this severity level carry higher uncertainty.
- The dataset was collected at a single institution in the 1980s. It does not represent global demographic diversity and may reflect historical clinical biases.
- The `thal` feature encodes thallium stress test results using a legacy categorical encoding that is inconsistent with modern nuclear cardiology reporting standards.
- This model has not been validated against external datasets or prospective clinical cohorts.

---

## Learning Takeaways

- Handling class imbalance in multi-class settings with SMOTE, including the `k_neighbors` constraint imposed by small minority classes
- Why accuracy is an insufficient metric for imbalanced classification and how macro F1 addresses this
- The difference between a model's raw output scores and calibrated probabilities, and why calibration matters in risk-sensitive applications
- Structuring a decoupled ML system with a separate API layer (FastAPI) and a frontend (Streamlit) that communicates with it over HTTP
- Sklearn's `Pipeline` abstraction for bundling preprocessing and model inference into a single serialisable object
