...
print("\n--- Tuning Random Forest Hyperparameters via Macro F1 ---")
X_scaled = scaler.fit_transform(X)

# SMOTE adjustment to handle minority class size constraint (Class 4 has only 13 samples)
smote = SMOTE(k_neighbors=2, random_state=42)
X_balanced_train, y_balanced_train = smote.fit_resample(X_scaled, y_original)
...
grid_search = GridSearchCV(
    ...
    cv=skf,
    scoring='f1_macro', # Changing the primary metric engine to target the best possible Macro F1 score
    n_jobs=-1
)
