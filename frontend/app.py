...
# ─────────────────────────────────────────────
# Inference + results (Unified Cloud Fail-Safe)
# ─────────────────────────────────────────────
...
if analyse:
    ...
    try:
        ...
        probabilities = np.array(result["probabilities"])
        max_prob = probabilities[prediction]
        st.success("Successfully connected to live FastAPI backend container!")
        
    except requests.exceptions.ConnectionError:
        # FALLBACK: Gracefully handle cloud deployment when backend is hosted locally on your Mac
        st.warning("⚠️ Backend service container offline. Running in Demo Mode with pre-loaded simulation metrics.")
        
        # Simulating a realistic prediction vector based on inputs (e.g. Higher age/ST depression logs higher risk)
        if oldpeak >= 2.0 or ca >= 2:
            prediction = 3 # Severe
            probabilities = np.array([0.10, 0.15, 0.20, 0.45, 0.10])
        else:
            prediction = 0 # Low Risk
            probabilities = np.array([0.75, 0.12, 0.08, 0.04, 0.01])
